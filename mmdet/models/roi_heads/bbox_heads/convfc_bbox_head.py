...
class Shared2FCBBoxHead(ConvFCBBoxHead):
    ...
    #             equal_fac = 1.0
    #             mean_overlaps = torch.mean(overlaps)
    #             mean_overlaps = torch.full_like(overlaps, mean_overlaps.data.detach().item(), device=overlaps.device)
    #             # equal_point = 0.85
    #             # equal_point = torch.full_like(overlaps, equal_point, device=overlaps.device)
    #             center_reduce_rate = 2.0
    #             center_reduce_rate = torch.full_like(overlaps, center_reduce_rate, device=overlaps.device)
    #             center_weight_min = 0.7
    #             center_weight_min = torch.full_like(overlaps, center_weight_min, device=overlaps.device)
    #             tmp_weight = 2 / (1 + torch.exp(center_reduce_rate * (overlaps - equal_fac * mean_overlaps)))
    #             center_weight = torch.max(tmp_weight, center_weight_min)
    #             wh_increase_rate = 2.0
    #             wh_reduce_rate = torch.full_like(overlaps, wh_increase_rate, device=overlaps.device)
    #             wh_weight_max = 1.3
    #             wh_weight_max = torch.full_like(overlaps, wh_weight_max, device=overlaps.device)
    #             tmp_weight = 2 / (1 + torch.exp(wh_reduce_rate * (equal_fac * mean_overlaps - overlaps)))
    #             wh_weight = torch.min(tmp_weight, wh_weight_max)
    #             iou_adaptive_weight = torch.stack([center_weight, center_weight, wh_weight, wh_weight], dim=1)
    ...
    #             overlaps = self.iou_calculator(decode_target_bbox, decode_pred_bbox, is_aligned=True)
    #             mean_overlaps = torch.mean(overlaps)
    #             overlaps = torch.full_like(overlaps, mean_overlaps.data.detach().item(), device=overlaps.device)
    #             equal_point = 0.85
    #             equal_point = torch.full_like(overlaps, equal_point, device=overlaps.device)
    #             center_reduce_rate = 1.2
    #             center_reduce_rate = torch.full_like(overlaps, center_reduce_rate, device=overlaps.device)
    #             center_weight_min = 0.25
    #             center_weight_min = torch.full_like(overlaps, center_weight_min, device=overlaps.device)
    #             tmp_weight = 1 / (1 + torch.exp(center_reduce_rate * (overlaps - equal_point)))
    #             center_weight = torch.max(tmp_weight, center_weight_min)
    #             wh_increase_rate = 2
    #             wh_reduce_rate = torch.full_like(overlaps, wh_increase_rate, device=overlaps.device)
    #             wh_weight_max = 0.75
    #             wh_weight_max = torch.full_like(overlaps, wh_weight_max, device=overlaps.device)
    #             tmp_weight = 1 / (1 + torch.exp(wh_reduce_rate * (equal_point - overlaps)))
    #             wh_weight = torch.min(tmp_weight, wh_weight_max)
    #             iou_adaptive_weight = torch.stack([center_weight, center_weight, wh_weight, wh_weight], dim=1)
    #             # iou_adaptive_weight = torch.cat([(1-overlaps).unsqueeze(-1), (1-overlaps).unsqueeze(-1), overlaps.unsqueeze(-1), overlaps.unsqueeze(-1)], dim=-1)
    #             losses['loss_bbox'] = self.loss_bbox(
...
@HEADS.register_module()
class DecoupleClsRegHead(ConvFCBBoxHead):
    def __init__(self, fc_out_channels=1024, *args, **kwargs):
        super(DecoupleClsRegHead, self).__init__(
            num_shared_convs=0,
            num_shared_fcs=2,
            num_cls_convs=0,
            num_cls_fcs=0,
    ...
    def forward(self, bbox_reg_feats, bbox_cls_feats):
        bbox_reg_feats = bbox_reg_feats.flatten(1)
        bbox_cls_feats = bbox_cls_feats.flatten(1)
        #cat_tmp = torch.cat([bbox_reg_feats, bbox_cls_feats], dim=0)
        for index in range(len(self.reg_fcs)):
            #cat_tmp = self.relu(self.shared_fcs[index](cat_tmp))
            #bbox_reg_feats = self.relu(self.reg_fcs[index](bbox_reg_feats))
            bbox_reg_feats = self.relu(self.reg_fcs[index](bbox_reg_feats))
            #bbox_cls_feats = self.relu(self.cls_fcs[index](bbox_cls_feats))
            bbox_cls_feats = self.relu(self.cls_fcs[index](bbox_cls_feats))
        #bbox_reg_feats, bbox_cls_feats = torch.chunk(cat_tmp, 2, 0)
        cls_score = self.fc_cls(bbox_cls_feats) if self.with_cls else None
        reg_pred = self.fc_reg(bbox_reg_feats) if self.with_reg else None
