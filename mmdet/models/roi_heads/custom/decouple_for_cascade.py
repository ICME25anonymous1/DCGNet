...
'''class SpatialAndChannelGlobalEnhance(nn.Module):
    def __init__(self, inchanels=256):
        ...
        self.conv1 = nn.Conv2d(2, 1, 1, 1)
        self.v_conv = nn.Conv2d(inchanels, inchanels, 1, 1)
        self.conv2 = nn.Linear(inchanels, inchanels//4)
        self.act1 = nn.ReLU()
        self.conv3 = nn.Linear(inchanels//4, inchanels)
        self.channel_mlp = nn.Sequential(
            nn.Conv2d(inchanels, inchanels // 4, 1),
    ...
    def forward(self, x):
        ...
        max_w_fea = torch.max(x, dim=2)[0]
        cat_fea = torch.cat([max_h_fea, max_w_fea], dim=-1).permute(0, 2, 1).contiguous()
        cat_fea = self.conv3(self.act1(self.conv2(cat_fea)))
        h_fea = cat_fea[:, :h, ...]
        w_fea = cat_fea[:, h:, ...].transpose(-1, -2)
        q = (h_fea @ w_fea).view(b, -1).unsqueeze(-1)
        q = F.softmax(q, dim=1)
...
'''class FeatureInteraction(nn.Module):
    def __init__(self, inchannels=256):
        ...
        self.conv1 = nn.Conv2d(inchannels * 3 * 2, inchannels * 3, 1, 1)
    ...
    def forward(self, center_fea, wh_fea, cls_fea):
        ...
        att = self.act1(self.conv1(max_mean_fea))
        ...
        center_out, wh_out, cls_out = att_list[0] * center_fea, att_list[1] * wh_fea, att_list[2] * cls_fea
...
class TwoFeatureInteraction(nn.Module):
    def __init__(self, inchannels=256):
        super(TwoFeatureInteraction, self).__init__()
        self.inchannels = inchannels
        self.max_pool = nn.AdaptiveMaxPool2d(1)
        self.mean_pool = nn.AdaptiveAvgPool2d(1)
        self.conv1 = nn.Conv2d(inchannels * 2 * 2, inchannels * 2 // 2, 1, 1)
        self.act1 = nn.Sigmoid()
        self.conv2 = nn.Conv2d(inchannels * 2 // 2, inchannels * 2, 1, 1)

    def forward(self, reg_fea, cls_fea):
        cat_fea = torch.cat([reg_fea, cls_fea], dim=1)
        max_fea = self.max_pool(cat_fea)
        mean_fea = self.mean_pool(cat_fea)
        max_mean_fea = torch.cat([max_fea, mean_fea],dim=1)
        att = self.conv2(self.act1(self.conv1(max_mean_fea)))
        att_list = torch.chunk(att, 2, 1)
        reg_out, cls_out = att_list[0] * reg_fea + reg_fea, att_list[1] * cls_fea + cls_fea
        return reg_out, cls_out

class GCEM(nn.Module):
    def __init__(self, inchanels=256):
        super(GCEM, self).__init__()
        self.adap_max = nn.AdaptiveMaxPool2d(1)
        self.adap_avg = nn.AdaptiveAvgPool2d(1)
        self.conv1 = nn.Conv2d(2, 1, 1, 1)
        self.v_conv = ConvModule(inchanels, inchanels, 1, 1, norm_cfg=dict(type='BN'))
        self.mlp = nn.Sequential(
            nn.Conv2d(inchanels, inchanels // 4, 1),
            nn.ReLU(),
            nn.Conv2d(inchanels // 4, inchanels, 1)
        )
        self.conv2 = ConvModule(1, 1, 1, 1, norm_cfg=dict(type='BN'))
    ...
    def forward(self, x):
        ...
        max_fea = self.adap_max(x)
        avg_fea = self.adap_avg(x)
        cross_spatial_fea = self.mlp(avg_fea)
        cat_fea = torch.cat([max_fea, avg_fea], dim=-1).permute(0, 3, 1, 2)
        tmp_att = self.conv1(cat_fea).squeeze(-1)
        channel_att = F.softmax(tmp_att, dim=-1)
        v = self.v_conv(x).view(b, c, -1)
        cross_channel_fea = (channel_att @ v).view(b, 1, h, w)
        cross_channel_fea = self.conv2(cross_channel_fea)
        refine_fea = cross_channel_fea.sigmoid() * cross_spatial_fea
        out = refine_fea + x
        return out
