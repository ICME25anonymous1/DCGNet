...
class Sobel2D(BaseModule):
    def __init__(self, in_channels=256):
        ...
        self.conv1 = nn.Conv2d(in_channels, in_channels, (1, 3), 1, (0, 1))
        self.conv2 = nn.Conv2d(in_channels, in_channels, (3, 1), 1, (1, 0))
        self.act = nn.ReLU()
    ...
    def forward(self, x):
        ...
        texture = self.act(self.conv2(self.conv1(texture)))
        # out = x + texture
        # return out
        return texture
...
class WaveRefineBlock(BaseModule):
    ...
    def forward(self, x, detail_feature):
        x = x + detail_feature
...
class DWTRefineBlock(BaseModule):
    ...
    @auto_fp16()
    def forward_feature(self, x, detail_feature):
        out = self.wave_refine(x, detail_feature)
        B, C, H, W = out.shape
    ...
    def forward(self, x, detail_feature):
        out = self.forward_feature(x, detail_feature)
        return out

@NECKS.register_module()
class DCEFPN(BaseModule):
    ...
    def __init__(self,
        ...
        super(DCEFPN, self).__init__(init_cfg)
        ...
        self.get_detail = Sobel2D()
        self.down = nn.ModuleList([nn.AvgPool2d(5, cur_stride, 2) for cur_stride in [2, 4]])
    ...
    def forward_feature(self, inputs):
        ...
        detail_feature = self.get_detail(laterals[0])
        #detail_feature = laterals[0]
        for i in range(used_backbone_levels - 1, 0, -1):
            ...
            if 'scale_factor' in self.upsample_cfg:
                ...
                #wavelets对上层特征进行精炼
                detail = self.down[i-2](detail_feature) if i > 1 else detail_feature
                refined_feature = self.feature_refine[i - 1](upsample_feature, detail)
                laterals[i - 1] = laterals[i - 1] + refined_feature
                #自适应的融合
